...
export default function AdminDashboard() {
  ...
  const { data: latestReqs, isLoading: isReqsLoading } = useQuery<
    ApiResponse<Page<PendingRequest>>
  >({
  ...
  const summary = summaryData?.result || ({} as Partial<SummaryData>);
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-top-4 duration-1000 p-1 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <StatCard
          ...
          color="rose"
          trend="Action Needed"
          isAlert={(summary.pendingRequests || 0) > 0}
        />
        <StatCard
          label="Đang mượn"
          value={summary.activeBorrowing || 0}
          icon={<GitPullRequest />}
          color="emerald"
          trend="In Use"
        />
        <StatCard
          label="Báo lỗi / bảo trì"
          value={
            (summary.brokenAssets || 0) + (summary.maintenanceAssets || 0) || 0
          }
          icon={<AlertCircle />}
          color="amber"
          trend="Warning"
          ...
          <div className="bg-slate-900 rounded-[3rem] p-8 text-white shadow-2xl shadow-blue-900/40 flex-1 overflow-hidden relative">
            ...
            <div className="space-y-4 relative z-10 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
              {pendingList.length > 0 ? (
                pendingList.map((req: PendingRequest) => (
                  <div
                    key={req.id}
                    ...
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className="flex gap-2 shrink-0">
                        <button
                          ...
                          className="p-2 bg-emerald-500/20 text-emerald-400 rounded-xl hover:bg-emerald-500 hover:text-white transition-all shadow-lg"
                        >
                        ...
                        <button
                          ...
                          className="p-2 bg-rose-500/20 text-rose-400 rounded-xl hover:bg-rose-500 hover:text-white transition-all shadow-lg"
                        >
                          <X size={14} />
                        </button>
                      </div>
          ...
          <div className="bg-white border border-slate-100 rounded-[2.5rem] p-8 shadow-sm">
            ...
            <div className="space-y-5">
              ...
              <TypeProgress
                ...
                value={
                  Math.round(
                    ((summary.brokenAssets || 0) / (summary.totalAssets || 1)) *
                      100,
                  ) || 0
                }
                color="bg-amber-500"
          ...
          <div className="bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full overflow-hidden border border-white">
            <div className="p-10">
              ...
              <form onSubmit={handleRejectSubmit} className="space-y-6">
                <textarea
                  autoFocus
                  className="w-full border-2 border-slate-100 rounded-[1.5rem] p-5 text-sm outline-none focus:border-rose-500/20 focus:ring-4 focus:ring-rose-500/5 min-h-[140px] font-medium"
                ...
                <div className="flex gap-4">
                  <button
                    type="button"
                    onClick={() => setRejectId(null)}
                    className="flex-1 py-4 rounded-2xl font-bold text-slate-500 bg-slate-100 text-xs uppercase"
                  >
                    Hủy
                  </button>
                  <button
                    ...
                    className="flex-1 py-4 rounded-2xl font-bold text-white bg-rose-600 text-xs uppercase shadow-xl shadow-rose-200"
                  >
...
function TypeProgress({ label, value, color }: TypeProgressProps) {
  return (
