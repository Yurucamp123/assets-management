...
export default function Profile() {
  // Lấy thông tin user từ store với type cụ thể
  ...
  const onSubmit = (data: ProfileFormData) => {
    updateProfile(data, {
      ...
      onError: (err) => {
        const axiosError = err as AxiosError<ApiErrorResponse>; // Ép kiểu thủ công bên trong
        toast.error(
          axiosError.response?.data?.message || "Có lỗi xảy ra khi cập nhật",
        );
      },
    });
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm text-center">
            ...
            <div
              className={`mt-4 inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-6">
              ...
              <div className="bg-amber-50 border border-amber-100 p-4 rounded-lg">
                <p className="text-xs text-amber-700 leading-relaxed">
                  <strong>Lưu ý:</strong> Tên đăng nhập (username) và Vai trò
                  được cấp định danh bởi hệ thống và không thể tự thay đổi. Vui
                  lòng liên hệ IT để hỗ trợ nếu có sai sót.
                </p>
              </div>
