...
import { AlertCircle } from "lucide-react"; // Import icon cảnh báo
        ...
        <div className="grid lg:grid-cols-2 items-center gap-10 max-w-6xl w-full mx-auto">
          ...
          <div className="border border-slate-300 rounded-lg p-8 bg-white shadow-lg max-w-md mx-auto w-full">
            <form className="space-y-4" onSubmit={handleSubmit}>
              <div className="mb-6">
                <h1 className="text-slate-900 text-3xl font-bold">Tạo tài khoản</h1>
                <p className="text-slate-500 text-sm mt-2">Bắt đầu quản lý thiết bị của bạn ngay hôm nay.</p>
              </div>
              ...
              {error && (
                <div className="flex items-center gap-2 bg-red-50 border border-red-200 text-red-600 px-4 py-3 rounded-lg text-sm font-medium animate-in fade-in duration-300">
                  <AlertCircle size={18} />
                  <span>
                    {(error as any)?.response?.data?.message || "Đăng ký thất bại. Vui lòng kiểm tra lại thông tin!"}
                  </span>
                </div>
              )}

              <div>
                <label className="text-slate-900 text-sm font-medium mb-2 block">Họ và tên</label>
                <input
                  name="fullName"
                  type="text"
                  required
