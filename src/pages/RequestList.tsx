...
export default function RequestList() {
  ...
  // 1. Lấy danh sách yêu cầu với Type-safe
  const { data, isLoading } = useQuery<ApiResponse<UserRequest>>({
    queryKey: ["my-requests", page],
    queryFn: () => requestApi.getMyHistory(page, pageSize),
  });

  const requests: UserRequest[] = data?.result?.content || [];
  const totalPages: number = data?.result?.totalPages || 1;

  // 2. Mutation gửi yêu cầu trả
  const returnMutation = useMutation({
    mutationFn: (id: number) => requestApi.requestReturn(id),
    onSuccess: () => {
  ...
  // 3. Render Status Badge với mapping an toàn
  const StatusBadge = ({ status }: { status: RequestStatus }) => {
    const config: Record<RequestStatus, StatusConfig> = {
      PENDING: {
    ...
    return (
      <div
        className={`flex items-center gap-1.5 px-3 py-1 rounded-full border text-[10px] font-black uppercase tracking-widest ${s.color}`}
      >
        {s.icon} {s.label}
      </div>
    );
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex justify-between items-end">
        <div className="space-y-1">
          ...
          <p className="text-slate-500 font-medium">
            Theo dõi và quản lý các thiết bị bạn đang sử dụng.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {requests.length > 0 ? (
          requests.map((req: UserRequest) => (
            <div
              key={req.id}
              className="bg-white rounded-[2.5rem] border border-slate-100 p-8 shadow-sm hover:shadow-2xl hover:shadow-blue-900/5 transition-all duration-500 group relative"
            >
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs font-mono text-slate-400">
                        REQ-{req.id.toString().padStart(5, "0")}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-6">
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100/50">
                  ...
                  <p className="text-sm font-black text-slate-700">
                    {new Date(req.borrowDate).toLocaleDateString("vi-VN")}
                  </p>
                </div>
                <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100/50">
                  <p className="text-[10px] font-bold text-slate-400 uppercase mb-2 flex items-center gap-1">
          ...
          <div className="col-span-full py-32 bg-white rounded-[3rem] border-2 border-dashed border-slate-100 flex flex-col items-center justify-center text-slate-300">
            ...
            <p className="text-sm">
              Hãy đăng ký mượn thiết bị để bắt đầu công việc.
            </p>
          </div>
        )}
      </div>
